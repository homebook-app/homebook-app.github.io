...
useHead({
  ...
  link: [
    ...
    { rel: 'stylesheet', href: 'https://fonts.googleapis.com/css2?family=Zain:wght@400;700&display=swap' }
  ],
  htmlAttrs: {
    lang: 'en'
  }
    ...
    <header class="sticky top-0 z-30 border-b border-default bg-default/80 backdrop-blur">
      <UContainer class="py-2 md:py-0 md:h-16 flex flex-wrap items-center justify-between gap-2 md:gap-6">
        ...
        <nav class="w-full md:w-auto order-3 md:order-none overflow-x-auto">
          <div class="flex items-center gap-1 min-w-max">
            <NuxtLink v-for="item in navItems" :key="item.to" :to="item.to"
              class="px-3 py-2 rounded-md text-sm transition-colors" :class="route.path === item.to || route.path.startsWith(`${item.to}/`)
        ...
        <div class="flex items-center gap-1 md:ml-auto">
          <UButton to="https://github.com/homebook-app/homebook" target="_blank" icon="i-simple-icons-github"
    ...
    <footer class="border-t border-default mt-10 relative z-10">
      <UContainer class="py-6 flex flex-wrap justify-between items-center gap-3 text-sm text-muted">
        ...
        <div class="flex items-center gap-4">
          <NuxtLink to="/releases" class="hover:text-highlighted transition-colors">
            Releases
          </NuxtLink>
